import Foundation
import MiniTui
import PiSwiftCodingAgent

// 32x32 RGB image of dax, hex encoded (3 bytes per pixel)
private let daxHex =
    ""

private let imageWidth = 32
private let imageHeight = 32

private func parseImage() -> [[[UInt8]]] {
    var pixels: [[[UInt8]]] = []
    let hex = Array(daxHex.utf8)
    for y in 0..<imageHeight {
        var row: [[UInt8]] = []
        for x in 0..<imageWidth {
            let idx = (y * imageWidth + x) * 6
            let r = parseHexByte(hex, idx)
            let g = parseHexByte(hex, idx + 2)
            let b = parseHexByte(hex, idx + 4)
            row.append([r, g, b])
        }
        pixels.append(row)
    }
    return pixels
}

private func parseHexByte(_ hex: [UInt8], _ offset: Int) -> UInt8 {
    let hi = hexVal(hex[offset])
    let lo = hexVal(hex[offset + 1])
    return hi &<< 4 | lo
}

private func hexVal(_ c: UInt8) -> UInt8 {
    if c >= UInt8(ascii: "0") && c <= UInt8(ascii: "9") {
        return c - UInt8(ascii: "0")
    }
    if c >= UInt8(ascii: "a") && c <= UInt8(ascii: "f") {
        return c - UInt8(ascii: "a") + 10
    }
    if c >= UInt8(ascii: "A") && c <= UInt8(ascii: "F") {
        return c - UInt8(ascii: "A") + 10
    }
    return 0
}

private func rgb(_ r: UInt8, _ g: UInt8, _ b: UInt8, bg: Bool = false) -> String {
    "\u{001B}[\(bg ? 48 : 38);2;\(r);\(g);\(b)m"
}

private let resetCode = "\u{001B}[0m"

private func buildImage() -> [String] {
    let pixels = parseImage()
    var lines: [String] = []

    // Use half-block chars: ▄ with bg=top pixel, fg=bottom pixel
    var row = 0
    while row < imageHeight {
        var line = ""
        for x in 0..<imageWidth {
            let top = pixels[row][x]
            let bottom = row + 1 < imageHeight ? pixels[row + 1][x] : top
            line += rgb(bottom[0], bottom[1], bottom[2]) + rgb(top[0], top[1], top[2], bg: true) + "▄"
        }
        line += resetCode
        lines.append(line)
        row += 2
    }
    return lines
}

@MainActor
public final class DaxnutsComponent: Component {
    private weak var tui: TUI?
    private let image: [String]
    private var timer: DispatchSourceTimer?
    private var tick = 0
    private let maxTicks = 25 // ~2 seconds at 80ms
    private var cachedLines: [String] = []
    private var cachedWidth = 0
    private var cachedTick = -1

    public init(tui: TUI) {
        self.tui = tui
        self.image = buildImage()
        startAnimation()
    }

    public func invalidate() {
        cachedWidth = 0
    }

    private func startAnimation() {
        let source = DispatchSource.makeTimerSource(queue: .main)
        source.schedule(deadline: .now() + 0.08, repeating: 0.08)
        source.setEventHandler { [weak self] in
            guard let self else { return }
            self.tick += 1
            if self.tick >= self.maxTicks {
                self.stopAnimation()
            }
            self.cachedWidth = 0
            self.tui?.requestRender()
        }
        timer = source
        source.resume()
    }

    private func stopAnimation() {
        timer?.cancel()
        timer = nil
    }

    public func render(width: Int) -> [String] {
        if width == cachedWidth && cachedTick == tick {
            return cachedLines
        }

        let t = theme
        var lines: [String] = []

        let center: (String) -> String = { s in
            let visible = visibleWidth(s)
            let left = max(0, (width - visible) / 2)
            return String(repeating: " ", count: left) + s
        }

        lines.append("")

        // Scanline reveal effect: show rows progressively
        let revealedRows = min(
            image.count,
            Int(Double(tick) / Double(maxTicks) * Double(image.count + 3))
        )

        for i in 0..<image.count {
            if i < revealedRows {
                lines.append(center(image[i]))
            } else if i == revealedRows {
                let scanline = String(repeating: "▓", count: imageWidth)
                lines.append(center(rgb(100, 200, 255) + scanline + resetCode))
            } else {
                lines.append(center(String(repeating: " ", count: imageWidth)))
            }
        }

        lines.append("")

        // Fade in text after image is revealed
        let textPhase = max(0, Double(tick) - Double(maxTicks) * 0.6)
        if textPhase > 0 || tick >= maxTicks {
            lines.append(center(t.fg(.accent, "Free Kimi K2.5 via OpenCode Zen")))
            lines.append(center(t.fg(.success, "\"Powered by daxnuts\"")))
            lines.append(center(t.fg(.muted, "— @thdxr")))
        } else {
            lines.append("")
            lines.append("")
            lines.append("")
        }

        lines.append("")
        if textPhase > 2 || tick >= maxTicks {
            lines.append(center(t.fg(.dim, "Try OpenCode")))
            lines.append(center(t.fg(.mdLink, "https://mistral.ai/news/mistral-vibe-2-0")))
        } else {
            lines.append("")
            lines.append("")
        }
        lines.append("")

        cachedLines = lines
        cachedWidth = width
        cachedTick = tick
        return lines
    }

    public func dispose() {
        stopAnimation()
    }

    deinit {
        timer?.cancel()
    }
}
